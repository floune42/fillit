...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/10/04 17:00:01 by jemalhad          #+#    #+#             */
/*   Updated: 2017/10/04 17:00:30 by jemalhad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_resolve(t_tetri *list, char **tab, int size)
{
	...
	if (list == NULL)
		return (1);
	while (x < size - tmp->mima.min)
	{
		y = 0;
		while (y < size - tmp->mima.max)
		{
...
void			ft_create_min_max(t_tetri *node)
{
	...
	i = 0;
	node->mima.min = node->t[i].x;
	node->mima.max = node->t[i].y;
	while (i < 4)
	{
		i++;
		if (node->mima.min < node->t[i].x)
			node->mima.min = node->t[i].x;
		if (node->mima.max < node->t[i].y)
			node->mima.max = node->t[i].y;
	}
}
